...
import { ContainerStateManager } from '@/managers/detail/containerStateManager';
import { ContainerEvent } from '@workspace/typescript-interface/docker/docker.container';
import { ContainerLogsStateManager } from '@/managers/detail/containerLogsStateManager';
import { ContainerLogsEvent } from '@workspace/typescript-interface/docker/docker.container.logs';
import { ContainerStatsStateManager } from '@/managers/detail/containerStatsStateManager';
...
const containerManagerRegistry = new SingleResourceManagerRegistry(
    'Container',
    (resourceId, environmentId) => new ContainerStateManager(resourceId, environmentId),
);
...
app.get('/stream/:containerId', (c) => {
    ...
    return streamSSE(c, async (stream) => {
        ...
        let manager: ContainerStateManager;
        try {
            manager = await containerManagerRegistry.acquire(containerId, environmentId);
        } catch (err) {
        ...
        const cleanup = () => {
            ...
            containerManagerRegistry.release(containerId, environmentId);
        };
