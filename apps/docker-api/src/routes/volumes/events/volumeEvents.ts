...
import { VolumeStateManager } from '@/managers/detail/volumeStateManager';
...
const volumeManagerRegistry = new SingleResourceManagerRegistry(
    'Volume',
    (resourceId, environmentId) => new VolumeStateManager(resourceId, environmentId),
);
...
app.get('/stream/:volumeName', (c) => {
    ...
    return streamSSE(c, async (stream) => {
        ...
        let manager: VolumeStateManager;
        try {
            manager = await volumeManagerRegistry.acquire(volumeName, environmentId);
        } catch (err) {
        ...
        const cleanup = () => {
            ...
            volumeManagerRegistry.release(volumeName, environmentId);
        };
