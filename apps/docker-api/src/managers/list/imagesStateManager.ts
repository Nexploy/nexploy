...
import { containerImageEvents } from '@/managers/list/containersStateManager';
...
export class ImagesStateManager extends BaseStateManager {
    ...
    constructor(environmentId: string) {
        super({
            managerName: `Images State Manager [${environmentId}]`,
            environmentId,
    ...
    private async syncVersionDelete(oldState: Image): Promise<void> {
        ...
        if (!process.env.NEXPLOY_API_URL || !(await getNexployApiKey())) {
            logger.warn('Cannot sync version delete: NEXPLOY_API_URL or the Nexploy API key is not available');
            return;
        }
