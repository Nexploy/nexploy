...
import { hidesInfrastructureNetwork } from '@/lib/infrastructureGuard';
...
export class NetworksStateManager extends BaseStateManager {
    ...
    constructor(environmentId: string) {
        super({
            managerName: `Networks State Manager [${environmentId}]`,
            environmentId,
    ...
    private async refreshNetworkState(networkId: string): Promise<void> {
        ...
        if (hidesInfrastructureNetwork(newState)) return;
    ...
    private async buildNetworkMap(): Promise<Map<string, Network>> {
        ...
        for (const network of inspected) {
            ...
            if (hidesInfrastructureNetwork(state)) continue;
