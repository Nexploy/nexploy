...
export const GET = route
    ...
    .handler(async (request) => {
        try {
            const environmentId = new URL(request.url).searchParams.get('environment') ?? undefined;
            const diskUsage = await kyDocker.get('system/df', { environmentId } as KyDockerOptions).json<DiskUsage>();
