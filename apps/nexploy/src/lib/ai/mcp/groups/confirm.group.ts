        ...
        server.registerTool(
            'requestConfirmation',
            {
                ...
                inputSchema: z.object({
                    action: z.string().describe('Clear description of the destructive action you are about to perform'),
                    target: z.string().describe('The resource name, ID, or identifier that will be affected'),
                }).shape,
            },
            async ({ action, target }) => {
