...
function outputHandleIdx(node: Node | undefined, handleId: string | undefined): number {
...
export function computeGraphLayout(currentNodes: Node[], currentEdges: Edge[]): Map<string, { x: number; y: number }> {
    ...
    for (const node of mainNodes) {
        effHeights.set(node.id, node.measured?.height ?? FALLBACK_SIZE);
    }
