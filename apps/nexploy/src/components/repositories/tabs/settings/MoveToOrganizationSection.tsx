...
export function MoveToOrganizationSection({
    repositoryId,
    currentOrganizationId,
}: MoveToOrganizationSectionProps) {
    const t = useTranslations('repository.settings.organization');
    const { isAdmin } = usePermissions();
    const [selectedOrganizationId, setSelectedOrganizationId] = useState<string>('');

    const { data: organizations, isLoading } = useSWR<UserOrganization[]>(
        { url: '/api/organizations' },
        fetcherApi,
    );

    const { executeAsync, isPending } = useAction(
        moveRepositoryToOrganizationAction.bind(null, repositoryId),
    );
    ...
    const isDisabled =
        isLoading || isPending || !canMoveOut || targetOrganizations.length === 0;
