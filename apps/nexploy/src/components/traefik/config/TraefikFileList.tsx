...
import { TraefikNewFileDialog } from '@/components/traefik/config/TraefikNewFileDialog.tsx';
...
export function TraefikFileList() {
    ...
    return (
        <div className="flex h-full flex-1 flex-col overflow-hidden">
            <div className="flex h-9 items-center justify-between border-border border-b pr-1.5 pl-3">
                ...
                <ScrollAreaWithShadow orientation="both" className={'h-full'}>
                    <div
                        className={cn(
                            ...
                            rootHighlight && 'bg-primary/5 ring-1 ring-primary/40 ring-inset',
                        )}
                    ...
                    >
                        {tree.length === 0 ? (
                            <p className="px-2 py-4 text-center text-muted-foreground text-xs">{t('noFiles')}</p>
                        ) : (
                            <TreeProvider
                                animateExpand={false}
                                indent={14}
